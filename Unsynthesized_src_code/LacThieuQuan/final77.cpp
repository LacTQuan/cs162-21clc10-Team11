...
Course* find_course(Semester* sem, string CourseID) {
    if(sem->pCourse == NULL) return NULL;
    Course* pCur_Course = sem->pCourse;

    while(pCur_Course != NULL){
        if(pCur_Course->CourseID == CourseID) return pCur_Course;
        pCur_Course = pCur_Course->pNext_Course;
    }
    return NULL;
}
    ...
    while (sem) {
        ...
        while (fin.is_open() && !fin.eof()) {
            ...
            if (s==".") {
                ...
                if (s==id) {
                    string str="";
                    cout << " " << setfill(' ') << setw(8) << left << course_id << " | ";
...
void enroll_main(Year* &year, Semester* &semester,string studentID,Registration_Session &S){
    ...
    if (tmp->name != year->name) {
        ...
        //view all course
        string CourseID;
        cout << "Course's ID which you want to enroll in: "; cin >> CourseID;
        Course* course_regis = find_course(semester,CourseID);
...
void Create_mark_main(Year* year,Semester* sem){ // Import scoreboard
    string CourseID;
    cout << "Enter course's id to import its scoreboard: "; cin>>CourseID;
    Course* course_regis = find_course(sem,CourseID);
    if(course_regis == NULL){
        Textcolor(_ERROR);
        cout<<"Invalid course.\n";
        Textcolor(NORMAL);
...
void View_scoreboard_main(Semester* sem){
    string course_name;
    cout<<"Enter course name to view its scoreboard: ";
    cin.ignore(1000, '\n');
    getline(cin, course_name, '\n');
...
void update_student_result(Year* &year, Semester* &sem){
    ...
    //find course
    string course_name;
    cout << "Enter course name to update its scoreboard: ";
    cin.ignore(1000, '\n');
    getline(cin, course_name, '\n');
