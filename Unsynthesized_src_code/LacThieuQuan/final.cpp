...
void View_Class_Scoreboard_main(Class* classes){
    ...
    while (pStudent != NULL){
        ...
        cout<<endl;
        //print concept
        cout << pStudent->StudentID << "\t" << pStudent->LastName + " " + pStudent->FirstName << "\t";
...
void Student_View_Scoreboard(Year* year,string studentID){
    Student* student = find_student_in_many_classes(year, studentID);
    if(student->pCur_Cour == NULL){
        cout << "No courses are enrolled" << endl;
    }

    Cur_Course* pCur = student->pCur_Cour;
    cout << "Course's name\tMidterm mark\tFinal mark\tOther\tTotal" << endl;
    while(pCur != NULL){
        cout <<pCur->Course_Name<< "\t" << pCur->mark.Midterm << "\t" << pCur->mark.Final << "\t"
        << pCur->mark.Other << "\t" << pCur->mark.Total << endl;
        pCur = pCur->pNext_Cur_Cour;
    }    
}
		...
		if (_kbhit()) {
			...
			if (var == 72) { //len
				...
				if (cell == 1){
                    ...
                    system("cls");
                    Student_View_Scoreboard(year,username);
                    cin.ignore(1000, '\n');
                    getchar();
                }
                else if (cell == 5) {
			...
			if (var == 72) { //len
				...
				if (cell == 1) {
                    ...
                    if(import_sem == NULL){
					    ...
					    COURSE_STU_PAGE(username,year,import_sem,locate_x, locate_y, 6, COURSE_STU_name_list_of_boxs);
                    }
