...
void Print_Course(Semester* sem) {
    ...
    while (pCourse != NULL) {
        ...
        // End of table
        cout << setfill('_') << setw(11) << right << "|"; // lien
        cout << setw(10) << right << "|";
        cout << setw(11) << right << "|";
        cout << setw(23) << right << "|";
        cout << setw(31) << right << "|";
...
bool semExists(Year* year, string findSem){ // lien
...
void Create_Semester(Year *&year){
    ...
    do{ // lien
        exist = false;
        cout << "Enter a new semester of the year " << year->name << " (ex: 1, 2, 3) ";
        cin >> name_semester;
...
void Print_Student(Class *classes){
    cout << "List of students in class " << classes->name << endl; // lien
    Student *pCur_Student = classes->pStudent;
...
void Create_Year(Year* &year){
    ...
    do{
        exist = false;
        cout<<"Enter new school year (ex: 2021) "; // lien
        cin >> name_year;
...
void Create_Class(Year* &year){
    ...
    do{
        exist = false;
        cout<<"Enter a new class of the year "<< year->name << " (ex:21CLC10) "; // lien
        cin >> name_class;
...
Year* Import_Year(Year* year){
    ...
    do{
        pCur_Year = year;
        string find_year;
        cout<<"Enter a school year (ex: 2021) "; // lien
        cin>>find_year;
                        
        while(pCur_Year != NULL && find_year != pCur_Year->name)
            pCur_Year = pCur_Year->pNext_Year;
        if (pCur_Year == nullptr){ // lien
            Textcolor(_ERROR);
            cout << "Invalid year.";
            Textcolor(NORMAL);
            cin.ignore(1000, '\n');
...
Class* Import_Class(Class* classes){
    ...
    do{
        pCur_class = classes;
        string find_class;
        cout<<"Enter a class (ex:21CLC10) "; // lien
        cin>>find_class;
        while(pCur_class != NULL && find_class != pCur_class->name)
            pCur_class = pCur_class->pNext_Class;
        if (pCur_class == nullptr){ // lien
            Textcolor(_ERROR);
            cout << "Invalid class.";
            Textcolor(NORMAL);
            cin.ignore(1000, '\n');
...
Semester* Import_Semester(Semester* sem){
    ...
    do{
        pCur_sem = sem;
        string find_sem;
        cout<<"Enter a semester to view courses (ex: 1, 2, 3) "; // lien
        cin>>find_sem;   
        while(pCur_sem != NULL && find_sem != pCur_sem->name)
            pCur_sem = pCur_sem->pNext_Semester;
        if (pCur_sem == nullptr){ // lien
            Textcolor(_ERROR);
...
void Add_1_Student_main(Year* year,Class *&classes, Account* account_head) {
    ...
    bool exist = false;
    do {
        exist = false; // lien: delete system("cls");
    ...
    if(Check_Cur_Course(course_regis,student) == true && Check_quantity(student) == true &&
       Check_Session(course_regis,student) == true ){
        ...
        if(Check_Cur_Course(course_regis,student) == false)
            cout<<"You have enrolled this course.\n";
...
void YEAR_PAGE(Year* &year, Account* account_head, int locate_x, int locate_y, int number_of_cells, string YEAR_name_list_of_boxs[]) {
    Year* import_year;
...
// student
void COURSE_STU_PAGE(Year* &year,Semester* &sem, User U, int locate_x, int locate_y, int number_of_cells, string COURSE_name_list_of_boxs[]) { // lien: User
...
void SEMESTER_STU_PAGE(Year* &year, User U, int locate_x, int locate_y, int number_of_cells, string SEMESTER_STU_name_list_of_boxs[]) {
    Semester* import_sem;
			...
			if (var == 72) { //len
				...
				if (cell == 1) {
                    ...
                    if(import_sem == NULL){
					    ...
					    COURSE_STU_PAGE(year,import_sem,U,locate_x, locate_y, 6, COURSE_STU_name_list_of_boxs);
                    }
...
void YEAR_STU_PAGE(Year* &year, User U, int locate_x, int locate_y, int number_of_cells, string YEAR_name_list_of_boxs[]) {
    Year* import_year;
			...
			if (var == 72) { //len
				...
				if (cell == 1) {
                    ...
                    if(import_year == NULL){
					    ...
					    SEMESTER_STU_PAGE(import_year, U, locate_x, locate_y, 3, SEMESTER_name_list_of_boxs);
                    }	
					//
					if (HOME == 4) return;
					//
...
void CHANGE_PASS_PAGE(Year* &year, Account* account_head, User &U, int locate_x,int locate_y, int number_of_cells, string CHANGE_PASS_name_list_of_boxs[]){ // lien
			...
			if (var == 72) { //len
                ...
                if (cell == 1) {
                    if (isStaff(U.username)){
					    ...
					    YEAR_PAGE(year, account_head, locate_x, locate_y, 6, YEAR_name_list_of_boxs);
	
                    }else{
                        system("cls"); //student
                        string YEAR_STU_name_list_of_boxs[] = {"View year","Home","Return"};
                        YEAR_STU_PAGE(year, U, locate_x, locate_y, 3, YEAR_STU_name_list_of_boxs);
                    }				
				}else if(cell == 2){
                    system("cls");           
...
void LOGIN_PAGE(Year* &year, Account* account_head, User U, int locate_x,int locate_y, int number_of_cells, string LOGIN_name_list_of_boxs[]) { // lien: account, user
			...
			if (var == 72) { //len
				...
				if (cell == 1) {
                    ...
                    if (account_check(account_head, U)){
                        ...
                        CHANGE_PASS_PAGE(year, account_head, U, locate_x, locate_y, 3, CHANGE_PASS_name_list_of_boxs);
...
int main() {
	...
	LOGIN_PAGE(year, account_head, U, 60, 10, 2, LOGIN_name_list_of_boxs); // lien: account, user
